...
def svc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(SVR(), params, cv=5, scoring='neg_mean_absolute_error', verbose=0)
        else:
            classifier = SVR(C=10)
        classifier.fit(x_train, y_train)
        # print("test", y_test, classifier.predict(x_test))
        mae.append(mean_absolute_error(y_test, classifier.predict(x_test)))
        ...
        if search:
            ...
            classifier = SVR(C=10)
        classifier.fit(x_train, y_train)
        mae_val.append(mean_absolute_error(y_test, classifier.predict(x_test)))

    return np.mean(mae_val), np.mean(mae)
