...
CSV_PATH = "C:\Users\riakh\Downloads\archive\Titanic-Dataset.csv"
...
def _first_present(cols_map, names):
    """Return the original-cased column name for the first present alias."""
    for n in names:
        if n in cols_map:
            return cols_map[n]
    return None
...
def load_titanic_features(csv_path: str):
    ...
    X = pd.concat(
        [
            ...
            one_hot_emb.reset_index(drop=True),            
        ],
        axis=1,
    ).astype(np.float32)
